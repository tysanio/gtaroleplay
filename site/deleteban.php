...
if(isset($_GET['nid']) && !empty($_GET['nid']) AND isset($_GET['char']) || !empty($_GET['char'])){
    ...
    $search = mysqli_query($con,"SELECT IP FROM `blacklist` where Username = '".$nid."'") or die('Error: ' . mysqli_error($con)); 
    $match  = mysqli_num_rows($search);
       if($match > 0){
        $match = 1;
		$rowip = mysqli_fetch_array($search);
    }else{
...
?>
		...
		<?php if($match > 0 AND $adminz >= 2)
		{
				$deletename = mysqli_query($con,"DELETE FROM `blacklist` WHERE Username  LIKE '".$nid."'") or die('Error: ' . mysqli_error($con));
				$deleteip = mysqli_query($con,"DELETE FROM `blacklist` WHERE IP LIKE '".$rowip["IP"]."'") or die('Error: ' . mysqli_error($con));
				// Set id variable 
				//
				header("location:bans.php?char=".$char."");
		}
